...
class MassGridencrypt:
    def private_key_to_secret(self,private_key):
        secret, compressed = private_key_to_secret_check_compressed(private_key)
        return secret

    def secret_to_private_key(self,secret, compressed):
    ...
    def address_to_public_key(self,address):
    ...
    def signature_to_public_key(self,signature, message):
    ...
    def verify(self,signature, message, address):
        public_key = self.signature_to_public_key(signature, message)
        if not public_key:
            return False

        if self.public_key_to_address(public_key) == address:
            return True
        else:
            return False
    ...
    def _unpad(self,message, block_size):
        length = len(message)
        if length == 0:
            return message
    ...
    def encrypt(self,public_key, message):
        padded = self._pad(message, AES.block_size)
    ...
    def decrypt(self,private_key, message):
        secret = self.private_key_to_secret(private_key)
        if secret == None:
            return None
        ...
        return self._unpad(padded, AES.block_size)
